...
from app.services.safety import with_safety
...
def create_session(body: SessionCreateRequest):
    ...
    return with_safety(session_store.create(body.patient_info))
...
def list_sessions(
    ...
    return [with_safety(s) for s in
            session_store.list_sessions(patient_name=patient_name, limit=limit)]
...
def update_drug(session_id: str, drug_index: int, update: DrugEntryUpdate):
    ...
    return with_safety(session_store.update_drug(session_id, drug_index, corrected))
